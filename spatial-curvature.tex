...
The dimensionless parameter $\kappa$ governs the coupling strength: it measures the amount of spatial curvature generated per unit of temporal potential. The physical content of Eq.~(\ref{eq:causal_chain}) is as follows: energy density creates the temporal field through the Gauss-type equation~\cite{Gwak2026a},
\begin{equation}
\nabla^2 \Phi_t = -\rho_t, \qquad \rho_t = \frac{4\pi G}{c^4}\,T_{00},
\label{eq:gauss}
\end{equation}
and the temporal field in turn generates spatial curvature with strength $\kappa$. Note that when spatial curvature is present, the Laplacian is taken with respect to the curved spatial metric $g_{ij}$, and the resulting solution for $\Phi_t$ differs from the Euclidean-spatial case. The implications of this modification are discussed in Section~\ref{sec:gr_verification}.
...
In the causal structure of Eq.~(\ref{eq:causal_chain}), spatial curvature is generated by the temporal field alone, with no additional energy beyond what the temporal field provides. The spatial factor in the coordinate speed of light is therefore bounded by the temporal factor that produces it. Light plays a special role in this bound: in general relativity, light propagation determines the causal structure of spacetime~\cite{Hawking1973}. In the present framework, light ($v = c$) propagates without velocity suppression and therefore activates spatial geometry up to the bound set by its cause, the temporal field (Section~\ref{sec:regimes}). This bound corresponds to equal temporal and spatial factors:
\begin{equation}
...
The spatial metric perturbation $h_{ij} = (e^{2\Phi_t} - 1)\,\delta_{ij}$ is present in the metric for all probes. However, its dynamical effect on particle motion depends on the particle's velocity. For a massive particle with velocity $v$, the equation of motion follows from the geodesic equation of the metric~(\ref{eq:exact_metric}), with the schematic form:
$$
...
In the static analysis~\cite{Gwak2026a}, where spatial geometry is Euclidean, the temporal field solution is $\Phi_t = m/r$ with $m \equiv GM/c^2$, and the corresponding $g_t = e^{-m/r}$ agrees with the Schwarzschild temporal component $\sqrt{1-2m/r}$ to first order in $m/r$, with a second-order deviation. That deviation is now resolved: it arises from the modification of the field solution when spatial curvature is included in the metric.

When the spatial curvature $g_{ij} = e^{2\Phi_t}\,\delta_{ij}$ is present, the Laplacian in the field equation is modified accordingly, and the resulting solution differs from the Euclidean-spatial case. In isotropic coordinates $(\bar{r})$, the Schwarzschild metric takes the form $$ g_{00}^{\rm Schw} = -\left(\frac{1 - m/\bar{r}}{1 + m/\bar{r}}\right)^{\!2} = -\bigl(1 - 4\,U_I + 8\,U_I^2 - \cdots \bigr), $$ where $U_I \equiv m/\bar{r}$. The temporal potential of the present framework that reproduces this expansion is $\Phi_t = 2\,U_I$; that is, $U \equiv \Phi_t = 2\,m/\bar{r}$ in isotropic coordinates. Substituting into the exact metric~(\ref{eq:exact_metric}), $$ ds^2\big|_{\rm ext} = -e^{-2U}\,c^2\,dt^2 + e^{2U}\,\delta_{ij}\,dx^i\,dx^j, $$ the temporal component becomes $$ g_{00} = -e^{-2U} = -\bigl(1 - 2U + 2U^2 - \cdots \bigr), $$ which coincides with the Schwarzschild expression $-(1-4\,U_I+8\,U_I^2-\cdots)$ to $O(U^2)$ upon the identification $U = 2\,U_I$. The temporal metric of the present framework therefore matches the Schwarzschild solution at post-Newtonian order.
...
In the weak-field regime, both nonlinear contributions to Eq.~(\ref{eq:full_field_eq}) are subleading: the gradient term $\frac{1}{2}\,|\nabla\Phi_t|^2$ is $O(\Phi_t^2)$ relative to $\nabla^2\Phi_t$, and the source correction $e^{2\Phi_t} + 3\,\Phi_t\,e^{2\Phi_t} - 1$ is $O(\Phi_t)$. The field equation therefore reduces to the Gauss-type equation of the original temporal field framework~\cite{Gwak2026a},
$$
\nabla^2\Phi_t = -\rho_t.
$$
