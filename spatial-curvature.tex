...
In the static analysis~\cite{Gwak2026}, where spatial geometry is Euclidean, the temporal field solution is $\Phi_t = m/r$ with $m \equiv GM/c^2$, and the corresponding $g_t = e^{-m/r}$ agrees with the Schwarzschild temporal component $\sqrt{1-2m/r}$ to first order in $m/r$, with a second-order deviation. That deviation is now resolved: it arises from the modification of the field solution when spatial curvature is included in the metric.
